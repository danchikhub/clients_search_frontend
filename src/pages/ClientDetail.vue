<template>
    <div class="row q-col-gutter-sm q-mt-md q-px-md">
        <div class="col-12 col-sm-12 col-md-3 col-lg-2">
            <div>
                <q-card v-if="isIndividual" class="q-pa-none">
                    <q-card-section>
                        <img class="image" :src="clientPhoto" :class="{ 'cursor-pointer': true }" />
                    </q-card-section>
                    <q-card-section>
                        <div class="client-card-field">
                            <span class="client-card-field__label">ID</span>
                            <p class="client-card-field__value">{{ subject.partyId }}</p>
                        </div>
                        <div class="client-card-field">
                            <span class="client-card-field__label">ИНН</span>
                            <p class="client-card-field__value">{{ subject.inn }}</p>
                        </div>
                        <div class="client-card-field">
                            <span class="client-card-field__label">ФИО</span>
                            <p class="client-card-field__value">{{ individualFullName }}</p>
                        </div>
                        <div class="client-card-field">
                            <span class="client-card-field__label">Полное имя (латиницей)</span>
                            <p class="client-card-field__value">{{ individualFullNameLat }}</p>
                        </div>
                        <div class="client-card-field">
                            <span class="client-card-field__label">Дата рождения</span>
                            <p class="client-card-field__value">{{ individual.birthDate }}</p>
                        </div>
                ...
                <q-card v-if="!isIndividual" class="q-pa-none">
                    ...
                    <q-card-section>
                        ...
                        <div class="client-card-field">
                            <span class="client-card-field__label">Наименование</span>
                            <p class="client-card-field__value">{{ legalFullName }}</p>
                        </div>
                        <div class="client-card-field">
                            <span class="client-card-field__label">Сокращенное имя латиницей</span>
                            <p class="client-card-field__value">{{ legalFullNameLat }}</p>
                        </div>
        ...
        <div class="col-12 col-sm-12 col-md-9 col-lg-10">
            <div class="bg-white q-pa-md">
                ...
                <q-expansion-item v-if="isIndividual" label="Данные физ. лица"
                    header-class="bg-grey-4 text-black text-bold" class="q-mt-md" default-opened>
                    <q-form class="q-mt-md" greedy>
                        <div class="row q-col-gutter-sm q-pb-sm">
                            <q-input v-model="individual.lastName" label="Фамилия" outlined dense
                                class="col-12 col-sm-4" counter :maxlength="maxLength"></q-input>
                            <q-input v-model="individual.firstName" label="Имя" outlined dense class="col-12 col-sm-4"
                        ...
                        <div class="row q-col-gutter-sm q-pb-sm">
                            <q-input v-model="individual.birthDate" label="Дата рождения" class="col-12 col-sm-3"
                                type="date" outlined dense></q-input>
                            ...
                            <q-select v-model="individual.maritalStatus" label="Семейное положение"
                                class="col-12 col-sm-3" outlined dense :options="maritalStatusOptions" emit-value
                                map-options></q-select>
                            <q-select v-model="individual.nationality" label="Национальность" class="col-12 col-sm-3"
                                outlined dense :options="nationalities" option-label="nationality" option-value="id"
                                emit-value map-options></q-select>
                        </div>
                        <div class="row q-col-gutter-sm q-pb-sm">
                            <q-input v-model="individual.workPlace" label="Место работы" outlined dense
                                class="col-12 col-sm-4" :disable="workPlaceDisable" counter :maxlength="100"></q-input>
                            <div>
                ...
                <q-expansion-item v-if="!isIndividual" label="Данные юр. лица"
                    header-class="bg-grey-4 text-black text-bold" default-opened>
                    <q-form ref="legalForm" class="q-mt-md" greedy>
                        <div class="row q-col-gutter-sm q-pb-sm ">
                            <q-input v-model="legal.fullName" label="Наименование" outlined dense
                                class="col-12 col-sm-4" counter :maxlength="140"></q-input>
                        ...
                        <div class="row q-col-gutter-sm q-pb-sm">
                            <q-input v-model="legal.shortName" label="Сокращенное наименование" outlined dense
                                class="col-12 col-sm-4" :maxlength="70"></q-input>
                            <q-input v-model="legal.shortNameKg" label="Сокращенное наименование на кыргызском" outlined
                                dense class="col-12 col-sm-4" :maxlength="70"></q-input>
                            <q-input v-model="legal.shortNameLat" label="Сокращенное наименование латиницей" outlined
                                dense class="col-12 col-sm-4" :maxlength="70"></q-input>
                        </div>
                        <div class="row q-col-gutter-sm q-pb-sm">
                            <q-input v-model="legal.okpoCode" label="Код ОКПО" outlined dense class="col-12 col-sm-4"
                                :disable="workPlaceDisable" counter :maxlength="8"></q-input>
                            <q-input v-model="legal.socialFund" label="Регистрационный номер соц.фонда" outlined dense
                                class="col-12 col-sm-4" :disable="workPlaceDisable" counter :maxlength="14"></q-input>
                    ...
                    <q-form class="q-mt-md" greedy>
                        <div class="row q-col-gutter-sm q-pb-sm ">
                            <q-select v-model="subject.clientType" class="col-12 col-sm-3" label="Тип клиента"
                                emit-value map-options outlined dense :options="clientTypeOptions" option-label="label"
                ...
                <q-expansion-item label="Контакты" header-class="bg-grey-4 text-black text-bold" class="q-mt-md"
                    default-opened>
                    <q-form class="q-mt-md" greedy>
                    ...
                    <q-form greedy>
                        <div v-for="(document, index) in documents" :key='document.documentType'>
                            ...
                            <div class="row q-col-gutter-sm q-my-sm">
                                <q-select v-model="document.documentType" class="col-12 col-sm-3" label="Тип документа"
                                    dense outlined :options="documentTypeOptions" option-label="docName"
                                    option-value="id" emit-value map-options></q-select>
                            ...
                            <div class="row q-col-gutter-sm">
                                <q-input v-model="document.docDate" type="date" class="col-12 col-sm-3"
                                    label="Дата выдачи" dense outlined></q-input>
                ...
                <q-expansion-item label="Адрес" header-class="bg-grey-4 text-black text-bold" class="q-mt-md"
                    default-opened>
                    <q-form greedy>
                        <span class="block text-weight-bold text-grey-8 q-mt-md">Фактический адрес проживания</span>
                        <div class="row q-col-gutter-sm q-my-sm">
                            <q-select v-model="address.addressType" readonly class="col-12 col-sm-3" label="Тип адреса"
                                dense outlined :options="addressTypeOptions" emit-value map-options></q-select>
                            <q-select v-model="address.addressArea" class="col-12 col-sm-3" label="Область" dense
                                outlined :options="addressAreaOptions" option-label="areaName" option-value="id"
                                emit-value map-options></q-select>
...
import ClientBriefInfo from 'src/components/ClientBriefInfo.vue';
...
const getIndividualPersonImage = (gender, person) => {
    const birthDate = moment(person.birthDate, 'DD.MM.YYYY').toDate();
    const yearOfBirth = moment(birthDate).year();
    const today = moment().format('YYYY');

    const age = +today - yearOfBirth;

    if (age <= AgeOptionsForAvatar.child) {
        return `${gender}-1`;
    }

    if (age >= AgeOptionsForAvatar.teenager && age < AgeOptionsForAvatar.youthful) {
        return `${gender}-2`;
    }

    if (age >= AgeOptionsForAvatar.youthful && age < AgeOptionsForAvatar.adult) {
        return `${gender}-3`;
    }
...
const clientPhoto = computed(() => {
    const gender = individual.value?.isMale ? 'male' : 'female';
    if(!isIndividual.value) return `src/assets/no-image-legal.png`;
    return `src/assets/no-image-${getIndividualPersonImage(gender, individual)}.png`
})

const individualFullName = computed(() => {
    return `${individual.value?.firstName} ${individual.value?.lastName} ${individual.value?.secondName} `
})
const individualFullNameLat = computed(() => {
    return `${individual.value?.firstNameLat} ${individual.value?.lastNameLat}`
})

const legalFullName = computed(() => {
    return `${legal.value?.fullName}`
})
const legalFullNameLat = computed(() => {
    return `${legal.value?.shortNameLat}`
})
...
.client-card-field__label {
  font-size: 16px;
  color: #777;
}
.client-card-field__value {
  font-size: 14px;
  margin: 0;
}
.image {
